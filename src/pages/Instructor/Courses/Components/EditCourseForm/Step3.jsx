...
function Step3({ formik }) {
  ...
  useEffect(() => {
    if (imgData !== null) {
      formik.setFieldValue("promotionImage", imgData.course.coverImageUrl);
    }
  }, [imgData]);

  useEffect(() => {
    if (videoData !== null) {
      formik.setFieldValue("promotionVideo", videoData.url);
    }
  }, [videoData]);
  const handleDeleteImg = () => {
    axios
      .patch(
        BaseApi + `/course/${params[1]}?delete=coverImage`,
        {},
        {
          headers: {
  ...
  const handleDeleteVideo = () => {
    axios
      .patch(
        BaseApi + `/course/${params[1]}?delete=promotionalVideo`,
        {},
        {
          headers: {
            "Content-Type": "application/json",
