...
export default function Courses() {
  ...
  const handleOptionSelect = async (selectedOption) => {
    let response = await axios
      .get(`${BaseApi}/course/category/${selectedOption}/subCategory`)
      .catch((err) => {
        console.log(err);
      });
    setCourses(response.data.courses);
  ...
  return (
    <div className="container">
      <div className="header">
        <div className="img relative">
          <img
            ...
            src="https://cdn.pixabay.com/photo/2015/07/17/22/42/library-849797_640.jpg"
          ...
          <div className=" absolute h-full w-full top-0 bg-gray-900 bg-opacity-60 flex justify-center items-center">
            <div className="w-3/4">
              ...
              <p className=" text-1xl text-gray-300 ">
                "A comprehensive list of available courses covering various
                subjects, topics, and skill levels. Users can browse through the
                catalog to find courses that match their interests and learning
                goals."
              </p>
            </div>
          </div>
      ...
      <div className="flex py-3 my-4 w-full m-auto gap-2 overflow-y-auto">
        {category.map((item) => {
          return (
            <Box
              className=" w-full text-center m-auto p-2 rounded-md"
              sx={{
      ...
      {courses.length > 0 ? (
        <div className=" text-center">
          <div className="mx-auto text-left max-w-2xl  lg:max-w-7xl">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-6 gap-y-6  xl:gap-x-8">
              {courses.map((ele) => (
                <div
                  key={ele.id}
                  ...
                  >
                    ...
                    <div className="p-3">
                      ...
                      <div className="flex justify-between my-3 items-center">
                        ...
                        <h6 className="pt-1 px-2 font-bold">{ele.price} EGP</h6>
                      </div>
                    </div>
                  </Box>
                </div>
          ...
          <button
            className="my-3 bg-green-600  px-3 py-2  rounded-md"
            style={{ display: courses?.length < 9 ? "none" : "block" }}
            onClick={() => {
              NeedMore();
            }}
          >
